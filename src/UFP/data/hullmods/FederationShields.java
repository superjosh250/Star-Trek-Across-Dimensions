...
import UFP.data.util.EMPDeflectionManager; // Updated import
...
public class FederationShields extends BaseHullMod {

    public static final float ROTATION_RATE = 0.3f;
    public static final float SHIELD_UNFOLD_MULT = 2.4f;

    public static final Color FEDERATION_BAR_COLOR = new Color(96, 144, 158, 230);
    public static final Color FEDERATION_BORDER_COLOR = new Color(50, 50, 50, 200);
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ScriptPerformanceReader.startTrack("FederationShields.applyEffectsAfterShipCreation");
        try {
            ...
            ship.getMutableStats().getShieldUnfoldRateMult().modifyMult("federation_shield_unfold", SHIELD_UNFOLD_MULT);

            ShieldHitpointManager.initialize(ship);
            ShieldHitpointManager.registerDamageInterceptor(ship);
            ShieldHitpointManager.setupShieldHPUI(ship, FEDERATION_BAR_COLOR, FEDERATION_BORDER_COLOR);

            Map<String, Object> customData = ship.getCustomData();
            customData.put(UNIVERSAL_HP_PRESENCE_KEY, true);

            // Added the renamed manager listener
            ...
            ScriptPerformanceReader.endTrack("FederationShields.applyEffectsAfterShipCreation");
        }
    }

    @Override
    public void advanceInCombat(ShipAPI ship, float amount) {
        ScriptPerformanceReader.startTrack("FederationShields.advanceInCombat");
        try {
            ...
            ScriptPerformanceReader.endTrack("FederationShields.advanceInCombat");
        }
    }

    @Override
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ScriptPerformanceReader.startTrack("FederationShields.addPostDescriptionSection");
        try {
            ...
            ScriptPerformanceReader.endTrack("FederationShields.addPostDescriptionSection");
        }
    }

    public static float getConfiguredShieldHPOrFlux(ShipAPI ship) {
        ScriptPerformanceReader.startTrack("FederationShields.getConfiguredShieldHPOrFlux");
        try {
            return ShieldHitpointManager.getShieldHPForRefit(ship);
        } finally {
            ScriptPerformanceReader.endTrack("FederationShields.getConfiguredShieldHPOrFlux");
        }
    }

    protected String getGraphicStyle() {
        return UFP_CSV_Manager.getSelectedShieldStyle();
    }

    protected void applyVisuals(ShipAPI ship, ShieldAPI shield) {
        ScriptPerformanceReader.startTrack("FederationShields.applyVisuals");
        try {
            ...
            ScriptPerformanceReader.endTrack("FederationShields.applyVisuals");
        }
    }

    protected void applyHullStyleColors(ShipAPI ship, ShieldAPI shield) {
        ScriptPerformanceReader.startTrack("FederationShields.applyHullStyleColors");
        try {
            ...
            ScriptPerformanceReader.endTrack("FederationShields.applyHullStyleColors");
        }
    }
}