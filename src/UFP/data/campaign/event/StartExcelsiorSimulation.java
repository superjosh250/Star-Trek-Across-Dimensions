...
public class StartExcelsiorSimulation extends BaseCommandPlugin implements InteractionDialogPlugin {
    ...
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params, Map<String, MemoryAPI> memoryMap) {
        ...
        if (playerOwnsExcelsior()) {
            setFlag("$ufp_has_excelsior", true, globalMemory, targetMemory);
        ...
        simEnemyFleet = Global.getFactory().createEmptyFleet(Factions.NEUTRAL, "Klingon Squadron", true);
        if (location != null) {
        ...
        for (int i = 0; i < ENEMY_COUNT; i++) {
            FleetMemberAPI enemy = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DimensionsCrossedIDS.BOP_MISSION);
    ...
    public void backFromEngagement(EngagementResultAPI battleResult) {
        ...
        if (battleResult != null) {
            if (battleResult.getLoserResult() != null && battleResult.getLoserResult().getFleet() == simEnemyFleet) {
                destroyedCount += battleResult.getLoserResult().getDestroyed().size();
                destroyedCount += battleResult.getLoserResult().getDisabled().size();
            } else if (battleResult.getWinnerResult() != null && battleResult.getWinnerResult().getFleet() == simEnemyFleet) {
                destroyedCount += battleResult.getWinnerResult().getDestroyed().size();
                destroyedCount += battleResult.getWinnerResult().getDisabled().size();
            }
        }

        if (destroyedCount >= REQUIRED_KILLS) {
            FleetMemberAPI rewardShip = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DimensionsCrossedIDS.USS_EXCELSIOR);
            rewardShip.setShipName(SHIP_NAME_PLAYER);
            Global.getSector().getPlayerFleet().getFleetData().addFleetMember(rewardShip);
            setFlag("$ufp_sim_excelsior_won", true, globalMemory, targetMemory);
        } else {
            setFlag("$ufp_sim_excelsior_failed", true, globalMemory, targetMemory);
        }

        FireAll.fire(null, dialog, memoryMap, "PopulateOptions");
    }

    private boolean playerOwnsExcelsior() {
        CampaignFleetAPI fleet = Global.getSector().getPlayerFleet();
        if (fleet == null) return false;

        for (FleetMemberAPI member : fleet.getFleetData().getMembersListCopy()) {
            if (member == null) continue;
            if (DimensionsCrossedIDS.USS_EXCELSIOR.equalsIgnoreCase(member.getHullId())) return true;
            if (member.getVariant() != null && DimensionsCrossedIDS.USS_EXCELSIOR.equalsIgnoreCase(member.getVariant().getHullVariantId())) return true;
    ...
    private void clearFlags(MemoryAPI globalMem, MemoryAPI targetMem) {
        String[] keys = {"$ufp_has_excelsior", "$ufp_sim_excelsior_won", "$ufp_sim_excelsior_failed"};
        for (String key : keys) {
